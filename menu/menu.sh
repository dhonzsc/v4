...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/dhonzsc/myip/main/ipkita > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/dhonzsc/myip/main/ipkita | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/dhonzsc/myip/main/ipkita | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
Exp=$(curl -sS https://raw.githubusercontent.com/dhonzsc/myip/main/ipkita | grep $MYIP | awk '{print $3}')
fi
...
clear 
		    ...
		    echo -e "${Kcyab}│${NC}${gReeen}              Author : DONI${NC}"
                    echo -e "${Kcyan}└───────────────────────────────────────────┘${NC}"
                    echo -e "${Kcyan}┌───────────────────────────────────────────┐"
